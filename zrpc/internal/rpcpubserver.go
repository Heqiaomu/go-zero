...
import (
	...
	"github.com/zeromicro/go-zero/zrpc/pkg/discovery"
)
...
func NewRpcPubServer(etcd discov.EtcdConf, listenOn string, opts ...ServerOption) (Server, error) {
	registerEtcd := func() error {
		...
		// }
		// 自定义注册逻辑
		// pubOpts = append(pubOpts, discov.WithCustomKV(etcd))
		// pubClient := discov.NewPublisher(etcd.Hosts, etcd.Key, pubListenOn, pubOpts...)
		// return pubClient.B20KeepAlive()

		return discovery.RegisterService4zrpc(etcd.Hosts, etcd.GetExtra("key").(string), etcd.GetExtra("value").(string))
