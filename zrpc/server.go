...
func NewServer(c RpcServerConf, register internal.RegisterFn) (*RpcServer, error) {
	...
	metrics := stat.NewMetrics("0.0.0.0:" + strconv.Itoa(int(c.Port)))
	serverOptions := []internal.ServerOption{
		internal.WithMetrics(metrics),
		internal.WithStatInterceptor(c.StatInterceptor.RequestFilter, c.StatInterceptor.RequestContentFilter, c.StatInterceptor.ResponseContentFilter),
	}

	if c.HasEtcd() {
		server, err = internal.NewRpcPubServer(c.Etcd, "0.0.0.0:"+strconv.Itoa(int(c.Port)), serverOptions...)
		if err != nil {
			return nil, err
		}
	} else {
		server = internal.NewRpcServer("0.0.0.0:"+strconv.Itoa(int(c.Port)), serverOptions...)
	}
