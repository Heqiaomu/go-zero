...
func TestDailyRotateRuleOverCountFiles(t *testing.T) {
	var rule DailyRotateRule
	assert.Empty(t, rule.OverCountFiles())
	rule.maxBackup = 1
	rule.filename = "./logs/"
	assert.Empty(t, rule.OverCountFiles())
	rule.gzip = true
...
func TestDailyRotateRuleShallRotate(t *testing.T) {
	...
	var rule2 DailyRotateRule
	rule2.maxSize = 1
	rule2.currentSize = int64(rule2.maxSize * megabyte)
	assert.True(t, rule2.ShallRotate())

	var rule3 DailyRotateRule
	rule3.maxSize = 1
	rule3.currentSize = int64(rule3.maxSize*megabyte - 1)
	assert.False(t, rule3.ShallRotate())
}
