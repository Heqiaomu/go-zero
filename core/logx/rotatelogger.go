...
type (
	...
	RotateRule interface {
		...
		ShallRotate() bool
		SyncCurrentSize(size int64)
		OverCountFiles() []string
	}
...
func DefaultRotateRule(filename, delimiter string, days, maxSize, maxBackup int, gzip bool) RotateRule {
...
func (r *DailyRotateRule) BackupFileName() string {
	return fmt.Sprintf("%s%s%s", r.filename, r.delimiter, getNowTime())
}
...
func (r *DailyRotateRule) OutdatedFiles() []string {
	...
	boundary := time.Now().Add(-time.Hour * time.Duration(hoursPerDay*r.days)).Format(timeFormat)
...
// OverCountFiles returns the files that exceeded the backup number.
func (r *DailyRotateRule) OverCountFiles() []string {
	if r.maxBackup <= 0 {
		return nil
	}
	...
	var outdates []string
	for i := len(files) - r.maxBackup - 1; i >= 0; i-- {
		outdates = append(outdates, files[i])
	}
...
func (r *DailyRotateRule) ShallRotate() bool {
	return len(r.rotatedTime) > 0 && getNowDate() != r.rotatedTime ||
		(r.maxSize > 0 && int(r.currentSize) >= r.maxSize*megabyte)
}

// SyncCurrentSize sync current size
func (r *DailyRotateRule) SyncCurrentSize(size int64) {
	r.currentSize = size
	return
}
...
func (l *RotateLogger) maybeDeleteOverCountFiles() {
	files := l.rule.OverCountFiles()
	for _, file := range files {
		if err := os.Remove(file); err != nil {
			Errorf("failed to remove outdated file: %s", file)
		}
...
func (l *RotateLogger) write(v []byte) {
	if l.syncSize() && l.rule.ShallRotate() {
...
func (l *RotateLogger) syncSize() bool {
	if l.fp != nil {
		if fi, err := l.fp.Stat(); err == nil {
			l.rule.SyncCurrentSize(fi.Size())
		}
	}
	return true
}
