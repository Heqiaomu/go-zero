...
func SetUp(c LogConf) error {
	...
	// 只能选择一种日志类型，不满足需求，改造成多mode模式
	var errorSlice []string
	modes := strings.Split(c.Mode, logTypeSeparator)
	for _, mode := range modes {
		if err := setupWithType(strings.TrimSpace(mode), c); err != nil {
			errorSlice = append(errorSlice, err.Error())
		}
	}
	if len(errorSlice) > 0 {
		return errors.New(strings.Join(errorSlice, logErrorSeparator))
	}
	return nil
}

func setupWithType(mode string, c LogConf) error {
	switch mode {
	case consoleMode:
		setupWithConsole(c)
		return nil
...
// WithMaxSize customizes logging to automatically rotate the log files by max size.
...
// WithMaxBackup customizes logging to automatically rotate the log files by max backup count.
func WithMaxBackup(backup int) LogOption {
	return func(opts *logOptions) {
...
func createOutput(path string) (io.WriteCloser, error) {
	...
	return NewLogger(path, DefaultRotateRule(path, backupFileDelimiter, options.keepDays, options.maxSize, options.maxBackup,
...
func setupWithConsole(c LogConf) {
	...
	func() {
		...
		infoLog = newBlocfaceLog(infoLog, newLogWriter(log.New(os.Stdout, "", flags)))
		errorLog = newBlocfaceLog(errorLog, newLogWriter(log.New(os.Stderr, "", flags)))
		severeLog = newBlocfaceLog(severeLog, newLogWriter(log.New(os.Stderr, "", flags)))
		slowLog = newBlocfaceLog(slowLog, newLogWriter(log.New(os.Stderr, "", flags)))
		statLog = newBlocfaceLog(statLog, infoLog)
...
func setupWithFiles(c LogConf) error {
	...
	if c.MaxBackup > 0 {
		opts = append(opts, WithMaxBackup(c.MaxBackup))
	}
	...
	func() {
		...
		newInfoLog, err := createOutput(accessFile)
		if err != nil {
			return
		}
		infoLog = newBlocfaceLog(infoLog, newInfoLog)

		newErrorLog, err := createOutput(errorFile)
		if err != nil {
			return
		}
		errorLog = newBlocfaceLog(errorLog, newErrorLog)

		newSevereLog, err := createOutput(severeFile)
		if err != nil {
			return
		}
		severeLog = newBlocfaceLog(severeLog, newSevereLog)

		newSlowLog, err := createOutput(slowFile)
		if err != nil {
			return
		}
		slowLog = newBlocfaceLog(slowLog, newSlowLog)

		newStatLog, err := createOutput(statFile)
		if err != nil {
			return
		}
		statLog = newBlocfaceLog(statLog, newStatLog)
...
type blocfaceLog struct {
	logs []io.WriteCloser
}

func newBlocfaceLog(log io.WriteCloser, newlog io.WriteCloser) blocfaceLog {
	oldlog, ok := log.(blocfaceLog)
	if ok {
		oldlog.logs = append(oldlog.logs, newlog)
		return oldlog
	}

	return blocfaceLog{
		logs: []io.WriteCloser{newlog},
	}
}

func (blw blocfaceLog) Close() error {
	for _, wc := range blw.logs {
		wc.Close()
	}
	return nil
}

func (blw blocfaceLog) Write(data []byte) (int, error) {
	for _, wc := range blw.logs {
		wc.Write(data)
	}
	return len(data), nil
