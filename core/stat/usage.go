...
var allRefreshInterval = time.Minute

var cpuUsage int64

func init() {
	v := viper.New()
	v.SetConfigFile("conf/cfg.toml")
	v.ReadInConfig()
	v.SetDefault("log.stat.all_refresh_interval", 10)
	usageRefreshInterval := v.GetInt("log.stat.all_refresh_interval")
	if usageRefreshInterval > 0 {
		allRefreshInterval = time.Minute * time.Duration((usageRefreshInterval))
	}
	go func() {
